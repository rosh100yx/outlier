...
`outlier` is a local-first AI coding audit tool: token tracker, authorship analyzer, deskilling detector, carbon footprint calculator for AI code, agent blast radius mapper, and git pre-commit hook — works with Claude Code, Cursor, Aider, Copilot, Codex, and any tool via `outlier watch`.
...
## Screenshots & demos
- **Audit receipt** — terminal screenshot
- **Share submenu** — X / Reddit / Slack / WhatsApp / Telegram / Threads / Facebook
- **Chat submenu** — ChatGPT + Perplexity prefilled prompt
- **Nyan Cat wait animation** — ASCII loop during audit wait
- **Watch + learn flows** — `outlier watch` and `outlier learn`
