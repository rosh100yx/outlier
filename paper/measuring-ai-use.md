...
### 3.1 Indicator 1 — Execution Share (Authorship via Line Survival)

Past ecosystem trackers have relied on parsing `Co-Authored-By` Git trailers to measure AI involvement. However, commit-level tracking is fundamentally flawed: squashed pull requests destroy the metadata, and a commit containing one line of AI code is weighted identically to one containing ten thousand lines.

We introduce a **blame-based, content-matched line survival** metric. The metric is defined as:

> **Execution Share = (AI-Authored Lines Surviving) / (Total Lines in HEAD)**

The local CLI instrument operates by parsing the developer's raw AI session transcripts (e.g., Claude Code logs) for `Edit` and `Write` events. It extracts the exact lines generated by the agent, hashes them, and cross-references them against `git blame` and `git ls-files` in the current `HEAD`. Crucially, this only counts lines that *survive* into the living artifact. Trivial lines (blank lines, bare brackets, <8 chars) and generated files (e.g., lockfiles) are strictly excluded from both sides to prevent ratio inflation.

For agents that do not leave parseable local logs (e.g., Copilot, Aider, Cursor), we introduce an effect-observer primitive (`outlier watch`). This brackets a coding session, captures all file changes occurring within that temporal window, and attributes the surviving lines to the active tool, rendering the execution signal entirely tool-agnostic. We retain the commit-trailer proxy solely as a fallback floor when transcript data is purged or unavailable.
...
### 4.1 Execution Share: True reliance is 62% of surviving lines

Using the fallback commit-proxy on a 160-commit history, we established an initial baseline of **75.6% AI authorship** [Observation]. However, when applying the rigorous blame-based execution tracking over the repository's **8.2K live lines of code**, the instrument revealed a true **Execution Share of 62%** (5.1K lines directly attributable to AI generation that survived into production). Model mix: 108 Opus-4.8, 18 Sonnet-4.6, 1 other.
