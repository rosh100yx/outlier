...
const SHARE_PLATFORMS: SharePlatform[] = [
  { value: 'x',         label: 'X / Twitter',           hint: 'Open tweet composer' },
  { value: 'reddit',    label: 'Reddit',                 hint: 'Open subreddit submit' },
  { value: 'slack',     label: 'Slack / Discord',        hint: 'Copy receipt to clipboard' },
  { value: 'whatsapp',  label: 'WhatsApp / Telegram',    hint: 'Open chat with receipt text' },
  { value: 'threads',   label: 'Threads',                hint: 'Open threads.net composer' },
  { value: 'facebook',  label: 'Facebook',               hint: 'Open share dialog' },
  { value: 'copy',      label: 'Copy and Share link',    hint: 'Copy receipt to clipboard' },
];

function buildShareQuotes(stats: AuditStats) {
  const quoteTemplate = MUSK_QUOTES[Math.floor(Math.random() * MUSK_QUOTES.length)] as string;
  const text = quoteTemplate
    .replace('{tokens}', stats.tokens)
    .replace('{cachePct}', stats.cachePct)
...
`;
  ...
  return { text: text.trim(), asciiArt: asciiArt.trim() };
}

export async function showShareMenu(stats: AuditStats): Promise<void> {
  const { select } = require('@clack/prompts');
  console.log('');
  const platform = await select({
    message: 'Share on:',
    options: SHARE_PLATFORMS.map(p => ({ value: p.value, label: p.label, hint: p.hint })),
  });

  if (typeof platform === 'symbol') return;

  const { text, asciiArt } = buildShareQuotes(stats);

  try {
    if (platform === 'x') {
      const tweet = encodeURIComponent(`Outlier Code Audit:\n\n"${text}"\n\nCode Yield: ${stats.yieldPct ? stats.yieldPct.toFixed(1) + '%' : 'N/A'}\nContext Tax: ${stats.cachePct}%\n\n#10xDeveloper #AI`);
      execSync(`open "https://twitter.com/intent/tweet?text=${tweet}"`);
      console.log(`\n ${pc.cyan('→')} Opening X/Twitter composer...`);
    } else if (platform === 'reddit') {
      const title = encodeURIComponent(`Outlier Code Audit — Code Yield ${stats.yieldPct ? stats.yieldPct.toFixed(1) + '%' : 'N/A'}, Context Tax ${stats.cachePct}%`);
      execSync(`open "https://www.reddit.com/r/LocalLLM/submit?title=${title}"`);
      console.log(`\n ${pc.cyan('→')} Opening Reddit submit page (paste the receipt when prompted).`);
    } else if (platform === 'slack') {
      execSync('pbcopy', { input: asciiArt });
      console.log(`\n ${pc.green('✓')} Receipt copied to clipboard.`);
      console.log(` ${pc.cyan('→')} Paste into Slack or Discord.\n`);
    } else if (platform === 'whatsapp') {
      const waText = encodeURIComponent(`${text}\n\nCode Yield: ${stats.yieldPct ? stats.yieldPct.toFixed(1) + '%' : 'N/A'} | Context Tax: ${stats.cachePct}%`);
      execSync(`open "https://wa.me/?text=${waText}"`);
      console.log(`\n ${pc.cyan('→')} Opening WhatsApp...`);
    } else if (platform === 'telegram') {
      const tgText = encodeURIComponent(`${text}\n\nCode Yield: ${stats.yieldPct ? stats.yieldPct.toFixed(1) + '%' : 'N/A'} | Context Tax: ${stats.cachePct}%`);
      execSync(`open "https://t.me/share/url?url=https://github.com/rosh100yx/outlier&text=${tgText}"`);
      console.log(`\n ${pc.cyan('→')} Opening Telegram...`);
    } else if (platform === 'threads') {
      const threadText = encodeURIComponent(`${text}\n\nCode Yield: ${stats.yieldPct ? stats.yieldPct.toFixed(1) + '%' : 'N/A'} | Context Tax: ${stats.cachePct}%`);
      execSync(`open "https://threads.net/intent/post?text=${threadText}"`);
      console.log(`\n ${pc.cyan('→')} Opening Threads composer...`);
    } else if (platform === 'facebook') {
      const fbQuote = encodeURIComponent(`${text}\n\nCode Yield: ${stats.yieldPct ? stats.yieldPct.toFixed(1) + '%' : 'N/A'} | Context Tax: ${stats.cachePct}%`);
      execSync(`open "https://www.facebook.com/sharer/sharer.php?quote=${fbQuote}"`);
      console.log(`\n ${pc.cyan('→')} Opening Facebook share dialog...`);
    } else if (platform === 'copy') {
      execSync('pbcopy', { input: asciiArt });
      console.log(`\n ${pc.green('✓')} Receipt copied to clipboard.`);
      console.log(` ${pc.cyan('→')} Paste it wherever you want.\n`);
    }
  } catch (e) {
    console.log(`\n ${pc.red('✗')} Could not open browser. Copy the receipt below and share it manually:\n`);
    console.log(pc.dim(asciiArt));
  }
}
